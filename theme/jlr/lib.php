 ...
 * the {@link theme_jlr_less_variables()} here because we need to create selectors or
 * alter existing ones.
 *
 * @param theme_config $theme The theme config object.
 * @return string Raw LESS code.
 */
function theme_jlr_extra_less($theme) {
    $content = '';
 ...
 */
function theme_jlr_less_variables($theme) {
    $variables = array();
    ...
    if (!empty($theme->settings->footertextcolor)) {
        $variables['footertextcolor'] = $theme->settings->footertextcolor;
    }
    if (!empty($theme->settings->h2textcolor)) {
        $variables['h2textcolor'] = $theme->settings->h2textcolor;
    }
    if (!empty($theme->settings->phovercolor)) {
        $variables['h2textcolor'] = $theme->settings->phovercolor;
    }
    if (!empty($theme->settings->fontsize)) {
        $variables['fontsize'] = $theme->settings->fontsize;
    }
    return $variables;
}
 ...
 */
function theme_jlr_process_css($css, $theme) {
    global $OUTPUT;

    // Set the background image for the logo.
    $logo = $OUTPUT->get_logo_url(null, 100);
    $css = theme_jlr_set_logo($css, $logo);
    ...
    $css = theme_jlr_set_customcss($css, $customcss);
 ...
 */
function theme_jlr_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_jlr_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM &&
            ($filearea === 'logo' || $filearea === 'smalllogo' || $filearea === 'backgroundimage')) {
        $theme = theme_config::load('jlr');
 ...
 */
function theme_jlr_set_customcss($css, $customcss) {
